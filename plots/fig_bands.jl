...
function fig_bands(name::String, nameS::String; nev = 2, Vz = 0.2,  dir = "data/Conductance", dirS = "data/States", colormap = cgrad(:PuOr_4) |> reverse, colorpsi = cgrad(:managua10)[2], colorL = cgrad(:managua10)[8], )
    path = "$(dir)/$(name).jld2"
    res = load(path)["res"]
    @unpack t, α, μ, a0 = res.system.chain_params

    pathS = "$(dirS)/$(nameS).jld2"
    resS = load(pathS)["res"]
    @unpack Ψs = resS
    ...
    band!(ax, [first(krng), last(krng)], -.2, .2; color = :black, alpha = 0.05)
    text!(ax, 0, 0; text = L"$$ Helical regime", align = (:center, :center))
    ...
    arrows!(ax, [kc], [0], [.27], [0.3]; linewidth = 4, color = colordown)
    scatter!(ax, [kc], [0]; color = (colordown, 0.4), markersize = 20)
    arrows!(ax, [-kc], [0], [.27], [-0.3]; linewidth = 4, color = colorup)
    scatter!(ax, [-kc], [0]; color = (colorup, 0.4), markersize = 20)


    Colorbar(fig[1, 2]; colormap, label = L"\text{Im}(E)", ticks = ([0, 1], [L"\gamma_0 - \gamma_y", L"\gamma_0 + \gamma_y"]), labelpadding = -30)
...
fig = fig_bands("base_wire_nh", "base_wire_nh_long")
save("plots/figures/fig_bands.pdf", fig)
fig

##

fig = fig_bands("base_wire_nh", "base_wire_nh_long_B"; nev = 2)
fig
...
function fig_bands(name::String, nameS::String; nev = 2, Vz = 0.2,  dir = "data/Conductance", dirS = "data/States", colormap = cgrad(:PuOr_4) |> reverse, colorpsi = cgrad(:managua10)[2], colorL = cgrad(:managua10)[8], )
    ...
    sorted_indices = sortperm(abs.(real.(E.(krng, 1, Vz))))
    kc1 = krng[sorted_indices[1]]
    kc2 = krng[sorted_indices[2]]
    kc3 = krng[sorted_indices[3]]
    kcs = [kc1, kc3]
    colordown = cgrad(colormap)|> last 
    colorup = cgrad(colormap)|> first


    arrows!(ax, kcs, [0, 0], [.27, .27], [0.3, -0.3]; linewidth = 4, color = colordown)
    scatter!(ax, kcs, [0, 0]; color = (colordown, 0.4), markersize = 20)
    arrows!(ax, -kcs, [0, 0], [.27, .27], [-0.3, 0.3]; linewidth = 4, color = colorup)
    scatter!(ax, -kcs, [0, 0]; color = (colorup, 0.4), markersize = 20)
...
fig = fig_bands("base_wire_nh", "base_wire_nh_long"; Vz = 0.01)
#save("plots/figures/fig_bands_pre.pdf", fig)
fig